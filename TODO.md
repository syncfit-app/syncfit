...
|---|---|---|---|---|---|
| B4 | Progresi minggu (`current_week`) otomatis pindah sendiri vs manual selamanya | Workout Page / Program | Todo | 1 (berikutnya) | Beda dari B11 — ini soal APAKAH `current_week` harus otomatis maju (misal setelah semua hari di minggu itu selesai), bukan soal independensi data antar minggu (itu sudah beres di B11). Perlu diskusi perilaku yang diinginkan dulu |
...
| B3 | Kalkulasi kalori lebih akurat (MET per jenis latihan) | Workout Page | Done | MET dibedakan per kategori exercise (Legs 6.0, Push/Pull 5.0, Core dinamis 4.0, Core statis/isometric 3.0), lookup nama→kategori otomatis dari `EXERCISE_DB` (`getMetValue()` baru di `workoutEngine.ts`). Dihitung sebagai rata-rata TERBOBOT berdasarkan jumlah SET yang benar-benar completed per exercise (bukan rata-rata flat per exercise), diterapkan ke total durasi sesi. Fallback MET 5.0 kalau exercise custom/diedit manual (tidak ada di `EXERCISE_DB`) atau kalau tidak ada satupun set completed. Diuji 3 skenario (Legs-berat vs Push-berat, keduanya beda bobot set; 0 set completed → fallback) — semua match perhitungan manual |