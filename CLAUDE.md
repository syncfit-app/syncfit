...
- `user_programs` — 1:1 per user (PK = `user_id`). Berisi `plan_data` (jsonb, array `DayPlan[]` — lihat tipe di `workoutEngine.ts`), `experience`, `days`, `goal`, `current_week` (hanya berubah manual lewat klik tombol minggu di UI, tidak otomatis), `updated_at` (berubah tiap kali plan disimpan APAPUN alasannya — dipakai untuk deteksi "cache lokal device sudah tidak valid"), `plan_reset_at` (cuma berubah saat generate ulang beneran dari nol — dipakai sebagai batas pencarian sesi lama saat sinkronisasi, supaya tukar hari/edit tidak menghapus histori centang).
