...
- `user_programs` — 1:1 per user (PK = `user_id`). Berisi `plan_data` (jsonb, **objek per-minggu** `{"1": DayPlan[], "2": DayPlan[], "3": DayPlan[], "4": DayPlan[]}` — bukan array flat lagi sejak refactor B11; format lama otomatis dimigrasi saat fetch pertama kali. Tipe `DayPlan` ada di `workoutEngine.ts`), `experience`, `days`, `goal`, `current_week` (cuma preferensi "terakhir buka minggu berapa" — pindah minggu TIDAK generate ulang/tulis `plan_data`, murni ganti tampilan), `updated_at` (berubah tiap kali plan_data disimpan APAPUN alasannya — dipakai untuk deteksi "cache lokal device sudah tidak valid"), `plan_reset_at` (cuma berubah saat generate ulang beneran dari nol/4-minggu-sekaligus — dipakai sebagai batas pencarian sesi lama saat sinkronisasi, supaya tukar hari/edit tidak menghapus histori centang).
- `workout_logs` — riwayat sesi workout selesai (per baris = 1 sesi). Diisi oleh `WorkoutView.handleEndSession`. Punya kolom `week` (integer, nullable — baris lama sebelum migrasi B11 tidak terisi) supaya centang/reps di 1 minggu tidak pernah kebaca sebagai milik minggu lain, walau nama harinya kebetulan sama.
